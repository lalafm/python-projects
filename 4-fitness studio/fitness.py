...
# Clean - remove rows without valid data
...
print('\nThe country with highest interest in workouts is', top_country)
print(geo)

# 4: Find, between Philippines and Malaysia which country has highest interest in home workouts
country_filt = words_geo[(words_geo['Country'] == 'Philippines') | (words_geo['Country'] == 'Malaysia')]
max_value = country_filt['home_workout_2018_2023'].max()
home_workout_geo = country_filt[country_filt['home_workout_2018_2023'] == max_value]['Country'].tolist()[0]
print('\nThe country with highest interest in home workouts is', home_workout_geo)
